...
def crop_search_window(bbox, frame):
    ...
    #----TODO (1) Rozszerzenie obaszru poszukiwania 
    ...
    # Sprawdzenie czy obraz wykracza poza zakres. 0 jak nie wykracza, -xmin to jelsi wykracza zminia sie znak ze wzgledu na offset i x_pad <> 0. Analogicznie xmax-frame
    x_pad = max(0, -xmin, xmax-frame_width)
    y_pad = max(0, -ymin, ymax-frame_height)

    if x_pad > 0 and y_pad > 0:
        frame = cv2.copyMakeBorder(frame, y_pad, x_pad, cv2.BORDER_REFLECT)
        xmin += x_pad
        xmax += x_pad
...
def random_warp(img):
    ...
    img_resized = cv2.resize(img_rot, img_shape[::-1])
...
def predict(frame, position, H):
    ...
    gi_complex = np.fft.ifft2(G)

    gi = np.abs(gi_complex)

    return gi

def update(frame, position, Ai, Bi, G, lr=0.125):
    patch = crop_search_window(position, frame)
    ...
    Ai = lr * G * Fi_conj + (1 - lr) * Ai
    Bi = lr * Fi * Fi_conj + (1 - lr) * Bi
...
def update_position(spatial_response, position):
    ...
    indices = np.where(spatial_response == max_val)

    dy = np.mean(indices[0])
    dx = np.mean(indices[1])
    ...
    new_y = position[0] + shift_y
    new_x = position[1] + shift_x
